// Poppins is really small for some reason so all font sizes here are increased by ~33%
"Scheme"
{
    "Fonts"
    {
        "Showcase_Speedo_Large"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "43"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Showcase_Speedo_Medium"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Showcase_Speedo_Small"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "16"
                "weight"            "0"
                "additive"          "0"
                "antialias"         "1"
                "dropshadow"        "0"
            }
        }
        // =================================================================================================
        // JumpHUD Fonts
        // =================================================================================================
        "Ingame_Large"	// Health, ammo, etc
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "43"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Ingame_Medium"	// Metal, stickies out, etc
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Ingame_MediumSmall"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "22"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Ingame_MediumTiny"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "16"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Ingame_Small"	// Heads label, sentry kills, etc
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "11"
                "weight"            "0"
                "additive"          "0"
                "antialias"         "1"
                "dropshadow"        "0"
            }
        }
        // poppins is aligned much higher for some reason making it unusable here
        "Ingame_Tiny"	// Item meter labels (inside item meter bar)
        {
            "1"
            {
                "name"              "Coolvetica Italic"
                "tall"              "6"
                "weight"            "0"
                "additive"          "0"
                "antialias"         "1"
                "dropshadow"        "0"
            }
        }
        "Time_Medium"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "22"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "Score_Small"	// arena player count
        {
            "1"
            {
                "name"              "renogare_soft_regular"
                "tall"              "20"
                "antialias"         "1"
            }
        }
        "Score_Medium"
        {
            "1"
            {
                "name"              "renogare_soft_regular"
                "tall"              "30"
                "antialias"         "1"
            }
        }
        "Score_Large"
        {
            "1"
            {
                "name"              "renogare_soft_regular"
                "tall"              "36"
                "antialias"         "1"
            }
        }
        "Score_ExtraLarge"
        {
            "1"
            {
                "name"              "renogare_soft_regular"
                "tall"              "48"
                "antialias"         "1"
            }
        }
        "Menu_Medium"	// class/team selection counts
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Menu_Small"	// Main Menu Categories
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "22"
                "weight"            "500"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "Menu_Tiny"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "13"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "TeamName"	// team names on scoreboard & win panels
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "KillFeed"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "13"
                "weight"            "0"
                "additive"          "0"
                "antialias"         "1"
                "dropshadow"        "0"
            }
        }
        "QuestName"	// Contract quest name ingame
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "15"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        // techno additional custom fonts
        "DamageFont"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "weight"            "400"
                "additive"          "0"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        // =================================================================================================
        // M0REHUD FONTS
        // =================================================================================================
        "m0refont9"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "12"
                "antialias"         "1"
            }
        }
        "m0refont10"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "13"
                "antialias"         "1"
            }
        }
        "m0refont10Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "13"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont11"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "15"
                "antialias"         "1"
            }
        }
        "m0refont11Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "15"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont12"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "16"
                "antialias"         "1"
            }
        }
        "m0refont12Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "16"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont14"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "18"
                "antialias"         "1"
            }
        }
        "m0refont14Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "18"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont16"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "21"
                "antialias"         "1"
            }
        }
        "m0refont16Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "21"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont18"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "18"
                "antialias"         "1"
            }
        }
        "m0refont18Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "24"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont18Outline"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "24"
                "antialias"         "1"
                "outline"           "1"
            }
        }
        "m0refont18Numbers"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "24"
                "antialias"         "1"
                "outline"           "1"
            }
        }
        "m0refont20"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "antialias"         "1"
            }
        }
        "m0refont24"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "32"
                "antialias"         "1"
            }
        }
        "m0refont24Shadow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "32"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "m0refont26"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "35"
                "antialias"         "1"
            }
        }
        "m0refont30"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "40"
                "antialias"         "1"
            }
        }
        "m0refont32"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "43"
                "antialias"         "1"
            }
        }
        "m0refont36"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "48"
                "antialias"         "1"
            }
        }
        "m0refont48"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "64"
                "antialias"         "1"
            }
        }
        "m0refont58"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "77"
                "antialias"         "1"
            }
        }
        //
        "Uber_Spec_7"	// SpectatorGUI Health
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "9"
                "antialias"         "1"
            }
        }
        "Uber_Spec_8"	// SpectatorGUI Health
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "11"
                "antialias"         "1"
            }
        }
        "Uber_Spec"	// SpectatorGUI Health
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "12"
                "antialias"         "1"
            }
        }
        "Uber_Spec_10"	// SpectatorGUI Health
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "13"
                "antialias"         "1"
            }
        }
        // =================================================================================================
        // DEFAULT TF2 FONTS
        // =================================================================================================
        "CenterPrintText"	// tempus speedo font
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "24"
                "weight"            "300"
                "antialias"         "1"
                "additive"          "1"
            }
        }
        "QuestObjectiveTracker_Desc"	// contract goals ingame
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "11"
                "weight"            "0"
                "antialias"         "1"
            }
        }
        "QuestObjectiveTracker_DescGlow"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "11"
                "weight"            "0"
                "antialias"         "1"
            }
        }
        "QuestObjectiveTracker_DescBlur"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "11"
                "weight"            "0"
                "antialias"         "1"
            }
        }
        "ItemTrackerScore_InGame"	// contract bar text ingame
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "9"
                "weight"            "0"
                "antialias"         "1"
            }
        }
        "ItemFontNameSmallest"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "11"
                "weight"            "500"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "ItemFontNameSmall"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "12"
                "weight"            "500"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "ItemFontNameLarge"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "16"
                "weight"            "500"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "ItemFontNameLarger"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "24"
                "weight"            "500"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "ScoreboardMedium"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "27"
                "weight"            "500"
                "range"             "0x0000 0x007F"
                "antialias"         "1"
                "additive"          "0"
            }
        }
        "TargetID"	// Jump mode targetid name
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "38"
                "antialias"         "1"
                // "dropshadow"								"1"
            }
        }
        "TargetID_Small"	// Casual mode targetid name, competitive spectator player names
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "22"
                "antialias"         "1"
                // "dropshadow"								"1"
            }
        }
        "TargetID_Medium"	// freezecam player name, winpanel player names
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "24"
                "antialias"         "1"
                // "dropshadow"								"1"
            }
        }
        "TargetID_Data"	// casual targetid date
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "22"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "TargetID_Data_Small"	// centered disguise weapon name
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "17"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "TargetID_Big"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "tall"              "38"
                "antialias"         "1"
                // "dropshadow"								"1"
            }
        }
        "TargetID_Minmode"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "38"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "TargetID_Big_Minmode"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "38"
                "antialias"         "1"
                "dropshadow"        "1"
            }
        }
        "TeamMenuBold"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "48"
                "weight"            "500"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "TeamMenu"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "56"
                "weight"            "600"
                "additive"          "0"
                "antialias"         "1"
            }
        }
        "TeamMenuSmall"
        {
            "1"
            {
                "name"              "Poppins Black Italic" [$WINDOWS]
                "name"              "Poppins" [$LINUX]
                "Tall"              "32"
                "weight"            "600"
                "additive"          "0"
                "antialias"         "1"
            }
        }
    }
}